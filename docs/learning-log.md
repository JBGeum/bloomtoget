...
## [#20] 로그아웃 후 access 토큰 처리 방침 결정과 TTL 단축 (2026-09-16)
- 문제: 로그아웃 처리 후에도 access 토큰이 최대 1시간 살아 있다(의도인지 누락인지 코드 상에서 판별 불가).
- 원인: 버그가 아니라 구조적으로 무상태 JWT를 고른 결과. JwtAuthenticationFilter가 DB를 한 번도 보지 않는다.
- 결정: 즉시 무효화를 하지 않고 access TTL을 3600 → 900으로 단축
- 근거: 조회 8회 → 32회(8시간 세션 가정) 대 최악 노출 창 60분 → 15분. 즉시 무효화는 요청당 조회 1회가 붙는데(위 가정에서 1008회) 그 비용을 잴 환경이 나중에 가서야 생긴다.
- 버린 대안: jti 블랙리스트 — access 토큰에 jti를 넣고 저장소를 만들어야 하며, 로그아웃으로는 공격자가 든 토큰을 끊지 못한다. tokenValidAfter — 유출 대응은 되지만 기기별 선택 로그아웃을 잃고, 매 요청 조회가 붙는다
- 한 일: access TTL 900으로 단축(3947be6), 방침 검증 E2E 2건 추가(24fcc8f), DisplayName 한글 관례 명시(e026ce1)
- 결과: 최악 노출 창 60분 → 15분. 전체 테스트 83건 통과(새 2건 포함).
- 막힌 곳: 15분은 관행에서 온 임의값이고 30분이 아닌 이유를 대지 못한다. 세션 길이 데이터가 없어 위 8회/32회도 가정이지 측정값이 아니다.
- 다음에 볼 것: 유출 대응은 로그아웃과 다른 방아쇠(비밀번호 변경, 모든 기기 로그아웃)를 갖는 별개 기능. 배포가 생기면 다시 본다. logout_RefreshTokenRejected는 현재 동작인 400을 고정했다. 401이 옳으므로 #18에서 깨진다.